...
process_country_ref(
  country_ref_who_custom = readxl::read_excel(
    here::here("data-raw/country_territory_data/WHO_REF_COUNTRY_CUSTOM_2019.xx.xx.xlsx")),
  country_ref_who = readxl::read_excel(
    here::here("data-raw/country_territory_data/WHO_REF_COUNTRY_turkiye_edit_2021.09.xx.xlsx"), skip = 1),
  sdg_regions = read.csv(
    here::here("data-raw/country_territory_data/sdg regions.csv")),
...
##
saveRDS(meta2, here::here("output", round_name, "meta_precrisis.rds"))
...
process_vr_wrapper(
  ...
  vr_data = readxl::read_excel(
    here::here("data-raw", "dependent_data",  "vital_registration_WHO_2022.05.11.xls")),
...
process_study_data(
  ...
  data_2019 = readxl::read_excel(
    here::here("data-raw","dependent_data","specialized_studies_MMEIG_2019.xx.xx.xlsx")) %>%
  ...
  data_2021 = readxl::read_excel(
    here::here("data-raw", "dependent_data", "specialised_studies_MMEIG_2022.05.23.xlsx")) %>% 
    dplyr::rename(year_end = `year_end `) %>%
    dplyr::select(-refid, -live_births, -include, -extraction_year) %>%
    dplyr::mutate(entry_year = 2022) %>%
...
process_survey_data(
  survey_data = readxl::read_excel(
    here::here("data-raw", "dependent_data", "survey_DHS&MICS_2022.06.14.xlsx"), guess_max = 1048576) %>%
...
process_miscellaneous_data(
  misc_data =  read.csv(
    here::here("data-raw", "dependent_data", "miscellaneous_MMEIG_2019.xx.xx.csv")) %>%
    dplyr::rename(final_pm = pm) %>%
    dplyr::bind_rows(readxl::read_xlsx(
      here::here("data-raw", "dependent_data", "miscellaneous_MMEIG_2022.07.15.xlsx")) %>% 
