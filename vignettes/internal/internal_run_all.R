...
for (iso_alpha_3_code in iso_alpha_3_codes) {
  fit_bmis(
    ...
    vrdata = vrdata %>% dplyr::filter(iso_alpha_3_code == !!iso_alpha_3_code),
    round_name = round_name,
    jags_settings = jags_settings_bmis(run_length),
    first_year = round_first_year,
    last_year = round_last_year,
...
pdf(here::here("output", round_name, "comparison_to_etsimates_m60.pdf"), 16,8)
pl
dev.off()
##########################################################################################################
...
summ <- MCMCvis::MCMCsummary(fit, params = pars_to_save) %>% 
...
temp <- summ %>%
  dplyr::filter(pars2 %in% pars_to_save)

write.csv(temp, row.names = FALSE, here::here("output", round_name, "bmat_global","summary.csv"))
...
##########################       Diagnostics     #########################################################
##########################################################################################################
fit <- readRDS(here::here("output", round_name, "bmis_global","jags_fit.rds"))
pars_to_save <- c(
  "etaworld.k",
  "sensworld",
  "specworld",
...
write.csv(temp, row.names = FALSE, here::here("output", round_name, "bmis_global","summary.csv"))
...
MCMCvis::MCMCtrace(fit,
                   ...
                   Rhat = TRUE,
                   filename = "mcmc_trace.pdf",
                   wd = here::here("output", round_name, "bmis_global")
)
##########################################################################################################
