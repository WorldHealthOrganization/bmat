...
tbl_vr_ex <- df_excluded %>%
  ...
  dplyr::mutate(
    completeness = completeness*100,
    usability_percentage = usability_percentage*100
  ) %>%
  dplyr::relocate(period) %>%
  dplyr::arrange(period)
...
tbl5 <- df %>%
  dplyr::filter(type != 'vr') %>% #might need to exclude other types shown in other sections
  dplyr::filter(include) %>%
  dplyr::select(maternal_deaths = env_mat_final,
                female_deaths = obs_env,
                final_pm,
  ...
  dplyr::mutate(
    ...
    maternal_pm = ifelse(definition == 'maternal', final_pm, NA),
    pregn_pm = ifelse(definition == 'pregn', final_pm, NA),
    mmr_adjusted = mmr_adjusted*100000,
    mmr = mmr*100000
  )  %>%
