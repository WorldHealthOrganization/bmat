...
#'
#' @param round_name
#' @param estimates_to_compare_la1
#' @param iso_alpha_3_codes
#'
#' @return
#' @export
#'
#' @examples
plot_bmat_global <- function(
  round_name = NULL,
  estimates_to_compare_la1,
  iso_alpha_3_codes
){
  main_path <- make_output_directory_return_path(round_name, iso_alpha_3_codes, global_run = FALSE, bmis_or_bmat = "bmat")
  pl <- list()
  for (iso_alpha_3_code in iso_alpha_3_codes) {
    pl[[iso_alpha_3_code]] <- plot_bmat_onecountry_sub(
      main_data_adjusted = readRDS(here::here(main_path, "main_data_adjusted.rds")) %>% dplyr::filter(iso_alpha_3_code == iso_alpha_3_code),
