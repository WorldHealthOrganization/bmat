...
plot_bmis_one_country <- function(
  ...
  first_year <- estimates$year_reference %>% min
  last_year <- estimates$year_reference %>% max
  ...
  pdata <- pdata %>%
    dplyr::filter(parameter %in% c( "gamma_tn", "gamma_tp", "gamma_fp", "gamma_fn", "gamma_mat_vr", "gamma_truemat_vr", "sens", "spec", "vradj")) 
  ...
  plot_bmis_one_country_core(pdata, breaks)
}
...
plot_bmis_one_country_core <- function(pdata,
  ...
  p <- pdata %>%
    ...
    ggplot2::scale_x_continuous(name = "reference year", breaks = breaks, labels = breaks, limits = c(breaks[1], breaks[length(breaks)]), guide = ggplot2::guide_axis(angle = 90)) +
