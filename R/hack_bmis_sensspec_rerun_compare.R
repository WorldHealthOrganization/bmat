    ...
    ))
{
  ...
  main_path_update <- make_output_directory_return_path(round_name_update, iso_alpha_3_code, global_run, "bmis")
  ...
  jags_fit <- readRDS(here::here(main_path_update, "jags_fit.rds"))
  

  
  estimates <- readRDS(here::here(main_path_update, "estimates.rds"))
  
  main_data_for_plots <-
    calculate_data_for_plots(ssdata = ssdata_filtered,
                             main_path = main_path_update)
